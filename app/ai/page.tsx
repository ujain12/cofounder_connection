...
export default function AIPlayground() {
  ...
  function clearImage() {
    setImage(""); setImagePreviewUrl(null); setImageAnswer("");
  }
    ...
    try {
      ...
      if (task === "coach") {
        const { data: chatRow } = await supabase
          .from("chats").select("id").eq("match_id", selectedMatchId).maybeSingle();
        let transcript = "";
        if (chatRow?.id) {
          const { data: msgs } = await supabase
            .from("messages").select("sender_id,body,created_at")
            .eq("chat_id", chatRow.id).order("created_at", { ascending: true }).limit(messagesLimit);
    ...
    <AppShell eyebrow="Tools" title="AI Playground">
      <div style={{ maxWidth: 800 }}>
        ...
        <div style={card}>
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {!isMultimodal && (
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {[
                    { checked: useAppData,    set: setUseAppData,    label: "Use my app data (profile, matches, connections)" },
                    { checked: useRecentChat, set: setUseRecentChat, label: "Include recent chat messages (for coaching / summaries)" },
                  ].map((item, i) => (
                    <label key={i} style={{ display: "flex", alignItems: "center", gap: 10, color: "var(--text-secondary)", fontSize: 13, cursor: "pointer", userSelect: "none" }}>
                      <input
                        type="checkbox"
            ...
            {task === "rewrite_profile" && (
              <div style={{ background: "var(--accent-soft)", border: "1px solid var(--accent-border)", borderRadius: "var(--radius)", padding: "12px 16px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div>
                  ...
                  <div
                    ...
                    style={{
                      border: `2px dashed ${dragOver ? "var(--accent)" : imagePreviewUrl ? "var(--accent-border)" : "var(--border)"}`,
                      borderRadius: "var(--radius-lg)",
                      background: dragOver ? "var(--accent-soft)" : imagePreviewUrl ? "transparent" : "var(--bg-deep, #f4f0e8)",
                  ...
                  >
                    {imagePreviewUrl ? (
                      <div style={{ position: "relative" }}>
                        <img src={imagePreviewUrl} alt="Preview" style={{ width: "100%", maxHeight: 280, objectFit: "contain", display: "block", borderRadius: "var(--radius-lg)" }}/>
                        <div style={{ position: "absolute", top: 10, right: 10, display: "flex", gap: 8 }}>
                          <button onClick={e => { e.stopPropagation(); document.getElementById("img-input")?.click(); }}
                            style={{ background: "rgba(255,255,255,0.92)", border: "1px solid var(--border)", borderRadius: 8, padding: "6px 12px", color: "var(--accent)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                            Change
                          </button>
                          <button onClick={e => { e.stopPropagation(); clearImage(); }}
                            style={{ background: "rgba(255,255,255,0.92)", border: "1px solid var(--border)", borderRadius: 8, padding: "6px 12px", color: "#c0394a", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                            Remove
                          </button>
                        </div>
                      </div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                        <div style={{ width: 48, height: 48, borderRadius: 14, background: "var(--accent-soft)", border: "1px solid var(--accent-border)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          <svg width="22" height="22" fill="none" viewBox="0 0 24 24" stroke="var(--accent)" strokeWidth={1.8}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5"/>
                          </svg>
                        </div>
                        <div>
                          <p style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)", marginBottom: 4 }}>Drop image here or click to browse</p>
                          <p style={{ fontSize: 12, color: "var(--text-muted)" }}>JPG, PNG, WebP — max 10MB. Charts, slides, resumes, screenshots.</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div>
                  ...
                  <textarea value={imageQuestion} onChange={e => setImageQuestion(e.target.value)} rows={3} style={textareaStyle} placeholder="What would you like to know about this image?"/>
                </div>
              </div>
            )}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <button onClick={run} disabled={isBusy} style={{
                padding: "11px 28px",
        ...
        <div style={card}>
          ...
          {outputText ? (
            <div style={{ background: "var(--bg-deep, #f4f0e8)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: 20 }}>
              <pre style={{ whiteSpace: "pre-wrap", fontSize: 13, color: "var(--text-primary)", lineHeight: 1.8, fontFamily: "inherit", margin: 0 }}>
                {outputText}
              </pre>
            </div>
          ) : (
